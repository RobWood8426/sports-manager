...
(defn scorekeeper-live
  ...
  (let [fmt (java.text.SimpleDateFormat. "d MMM yyyy HH:mm")
    ...
    (shared/doc-public "Live Scoring — SchoolScore"
                       ...
                       [:div.max-w-sm.mx-auto.p-4.flex.flex-col.gap-6
                        [:h2 (get-in fixture [:fixture/sport-template :sport-template/name] "Game")]
                        (when-let [ag (:fixture/age-group fixture)]
                          [:p.text-sm.text-gray-500 ag])
                        (when-let [v (:fixture/venue fixture)]
                          [:p.text-sm.text-gray-500 v])
                          ...
                          [:div.flex.gap-2
                           (for [pl period-labels]
                             [:a {:href (str "/score/" fixture-id "/live?scode=" scode-id "&period=" pl)
                              ...
                              pl])])
                        [:div.grid.grid-cols-3.gap-4.items-center
                         [:div.flex.flex-col.items-center.gap-2
                          [:span.text-sm.font-semibold.text-center team-a]
                          [:span.text-6xl.font-bold.tabular-nums (str (:a score))]
                          [:form {:method "post" :action base-action}
                           ...
                           [:button.btn.btn-lg.btn-circle.btn-success {:type "submit"} "+"]]
                          [:form {:method "post" :action base-action}
                           (shared/csrf-field)
                           [:input {:type "hidden" :name "scode-id" :value (str scode-id)}]
                           [:input {:type "hidden" :name "team" :value "a"}]
                           ...
                           (when active-period
                             [:input {:type "hidden" :name "period" :value active-period}])
                           [:button.btn.btn-lg.btn-circle.btn-error {:type "submit"} "−"]]]
                         [:div.text-2xl.text-gray-400.text-center "vs"]
                         [:div.flex.flex-col.items-center.gap-2
                          [:span.text-sm.font-semibold.text-center team-b]
                          [:span.text-6xl.font-bold.tabular-nums (str (:b score))]
                          [:form {:method "post" :action base-action}
                           (shared/csrf-field)
                           [:input {:type "hidden" :name "scode-id" :value (str scode-id)}]
                        ...
                        [:div.flex.flex-col.gap-3
                         [:form {:method "post" :action (str "/score/" fixture-id "/submit")}
                          (shared/csrf-field)
                          [:input {:type "hidden" :name "scode-id" :value (str scode-id)}]
                          [:button.btn.btn-error {:type "submit"
                                                  :onclick "return confirm('Submit final score? This cannot be undone.')"}
                           "Submit final score"]]]
                        (when-let [s (:fixture/start-at fixture)]
                          [:p.text-xs.text-gray-400.text-center (.format fmt s)])]
